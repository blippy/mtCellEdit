	...
	)
{
	...
	for ( tot = 0; ; tot ++ )
	{
		...
		if ( res < 0 )
		{
			fprintf ( stderr, "ERROR eleana_polymap_load - ID '%s' "
				" not found\n", id );
		...
		else
		{
			...
			{
				fprintf ( stderr, "ERROR eleana_polymap_load -"
					" ID '%s' not found\n", id );
	...
	mtkit_tree_scan ( indexSeatID->tree, tree_scan_missing_cb, NULL, 0 );
