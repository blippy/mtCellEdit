	...
	)
{
	...
	case POLYMAP_FILETYPE_EPS:
		return mtKit::set_filename_extension ( filename, "eps", NULL,
			NULL );

	case POLYMAP_FILETYPE_PDF:
		return mtKit::set_filename_extension ( filename, "pdf", NULL,
			NULL );

	case POLYMAP_FILETYPE_PNG:
		return mtKit::set_filename_extension ( filename, "png", NULL,
			NULL );

	case POLYMAP_FILETYPE_PS:
		return mtKit::set_filename_extension ( filename, "ps", NULL,
			NULL );

	case POLYMAP_FILETYPE_SVG:
		return mtKit::set_filename_extension ( filename, "svg", NULL,
			NULL );
...
void MainWindow::pressFileSaveMap ()
{
	...
	while ( 1 )
	{
		...
		if ( ! filename.isEmpty () )
		{
			...
			char * correct = get_correct_filename (
